...
void I2SStop( void )
{
  ...
  DAIValue = LPC_I2S->I2SDAI;
  DAIValue &= ~(0x01 << 15);
  LPC_I2S->I2SDAO = ((16-1)<<6) | (0x1<<4) | (0x1<<3) | 0x1;	/* Master */
...
uint32_t I2SInit( void ) 
{
  ...
  LPC_PINCON->PINSEL0 |= 0x00055500;
	...
	LPC_PINCON->PINSEL9	|= (1 << 26) ;	//P4.29 AS I2SMCLK
	LPC_I2S->I2STXRATE |= (1 << 8) | 1; //I2STXMCLK P4.29
//	LPC_I2S->I2STXMODE &= ~ 0x1;
...
//  LPC_I2S->I2SRXRATE = 0x241;
	LPC_I2S->I2STXBITRATE = ((SystemCoreClock/4)/(44100*2*16)) - 1 ;
	I2SStop();
